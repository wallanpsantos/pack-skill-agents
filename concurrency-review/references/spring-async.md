...
`SecurityContextHolder` is `ThreadLocal`-bound. Auth context is lost across thread boundaries.
...
> the ITL map at VT creation time — expensive at scale. Prefer explicit propagation or `DelegatingSecurityContext*`.
