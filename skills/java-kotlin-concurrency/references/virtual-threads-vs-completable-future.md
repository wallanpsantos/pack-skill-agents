...
> **Kotlin note:** the equivalent axis in idiomatic Kotlin is Virtual Threads (plain blocking Kotlin) vs Kotlin
> coroutines (`suspend`/`Flow`) vs `CompletableFuture` (rare in coroutine-first code, mostly at a Java-interop
> boundary). See `kotlin-coroutines.md` §1 for the decision and §6 for the interop bridge.
...
Result(await(user, deadline),await(order,deadline));
        }catch(
...
cancel(true)); // close() would otherwise wait past the deadline
        }
        }
```
...
CompletableFuture.supplyAsync(() ->userClient.
