...
- A pauseable, resumable computation storing call-stack frames (program counter, locals, operand stack) on the **Java
  heap**, not on native memory.
...
Result(user.get(5, SECONDS),order.
...
Executors.newFixedThreadPool(100,Thread.ofVirtual().
...
ThreadPoolExecutor(50,50,0,TimeUnit.SECONDS,
...
**Before VTs:** a fixed pool (e.g. 200 threads) gave implicit backpressure. **After VTs:** no implicit backpressure →
**it must be explicit**.
...
callApi(ctx, id));        // immutable record travels as a parameter
        }
```
...
ScopedValue.where(ERRORS, new ArrayList<>()).
...
expensiveTransform(item),ForkJoinPool.
