...
## 7. Kotlin note (JVM target)

Every rule above applies unchanged to Kotlin — `BigDecimal`, `@Version`, and the money value object pattern are JVM
facts, not Java-specific ones. The one Kotlin-specific trap:

```kotlin
// ❌ the `/` operator is sugar for divide(BigDecimal) with no scale/rounding — same trap as Java
val rate = amount / divisor          // throws ArithmeticException on a non-terminating decimal expansion

// ✅ explicit scale + RoundingMode, exactly like the Java call
val rate = amount.divide(divisor, 6, RoundingMode.HALF_EVEN)
```

The `+` and `-` operators are fine (they are exact, like `add`/`subtract`), but treat `/` — and `*` when a rounding
mode is required — as ordinary method calls that need explicit arguments, not operators to reach for in money code.
See `kotlin-coroutines.md` for Kotlin-specific *concurrency* concerns (coroutines, `Mutex`, structured concurrency)
around financial state — this file covers the arithmetic and persistence side, which does not change by language.
