...
try (var exec = Executors.newVirtualThreadPerTaskExecutor()) {
    try {
        for (Item item : items) {
            pending.add(exec.submit(() -> httpCall(item)));
        }
        List<Item> out = new ArrayList<>(pending.size());
        for (Future<Item> f : pending) {
...
try {
    ...
    f.cancel(true);                     // orTimeout/get timeout does NOT stop the computation by itself
    throw new ServiceException("cpu budget exceeded", e);
} catch (ExecutionException e) {
    throw new ServiceException("cpu task failed", e.getCause());
}
```
...
for (int i = 0; i < items.size(); i++) {
    ...
    if (i % 100 == 0) {
        Thread.yield(); // hint only: no bound, no fairness guarantee, no extra parallelism
    }
}
```
