...
// ✅ Virtual-thread executor for I/O-bound @Async on Java 21
@Bean(destroyMethod = "shutdown")
public Executor taskExecutor() {
...
## 7. Spring Boot 3.4.5 + Java 21: VT-Aware Architecture
...
```xml
<!-- server.xml — Tomcat 10.1+ (backported; requires Java 21) -->
<Executor name="vtExecutor" className="org.apache.catalina.core.StandardVirtualThreadExecutor" />
<Connector port="8080" protocol="HTTP/1.1" executor="vtExecutor" />
```

### Architecture Decision

In Spring Boot 3.4.5 with Java 21 + VT container:
...
```java
// ✅ In Spring Boot 3.4.5 + VT: synchronous service is fine
@Service
